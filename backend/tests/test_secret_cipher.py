"""
Tests for src.core.security.secret_cipher — Fernet-based encryption for per-agent API keys.

加密策略：
- 主密钥优先从环境变量 AGENT_OVERRIDE_MASTER_KEY 读取（base64 编码的 32 字节）
- 未设置时自动生成并持久化到 backend/var/.secret_key
...
def _reset_cipher_state(tmp_path, monkeypatch):
    ...
    # 隔离数据目录，避免污染真实 data/
    monkeypatch.setattr(
        "src.core.security.secret_cipher._DATA_DIR", tmp_path, raising=False
    )
    # 清除已缓存的 master key
...
class TestRoundtrip:
    def test_encrypt_decrypt_returns_original(self):
        from src.core.security.secret_cipher import encrypt_value, decrypt_value
...
class TestPlaintextCompatibility:
    ...
    def test_is_encrypted_only_for_prefixed(self):
        from src.core.security.secret_cipher import is_encrypted
