...
- 相同/区别技术特征、最接近现有技术和新颖性等级必须由检索分析 Agent 综合检索证据后判断。
- 工具相似度、关键词命中或评分只作为线索；不得把工具分数直接映射成 `high | medium | low`。
- 未获得可核验对比文件时，不能虚构 `related_prior_art`；应说明证据不足并降低 `overall_confidence`。
