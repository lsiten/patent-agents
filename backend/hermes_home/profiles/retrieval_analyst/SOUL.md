...
- **默认情况（target_country="中国"）**：优先使用当前工具真实支持且可核验的专利数据源，并以中国专利法和审查指南为标准评估；如果目标法域的官方源未接入、超时或无结果，必须如实记录为“数据源覆盖限制”，并继续使用 Google Patents 专利详情、USPTO、arXiv、官方网页或专业技术来源补强证据。不能把本地未配置某个数据源当成需求缺口或流程终止理由。
