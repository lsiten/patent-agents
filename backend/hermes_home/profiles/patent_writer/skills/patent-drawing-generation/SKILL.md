...
- 质量审查未指出问题且文件可访问的附图必须复用上一轮 `drawings` 元数据，不得重画、替换或清空。
- 只有当审查意见明确指出某张图缺失、损坏、重复、图文不符，或该图对应实施例/权利要求发生实质变化时，才重新调用 `patent_drawing_generator` 生成该图。
- 重新生成时只生成被点名的问题图；其他图继续原样返回，保证最终 JSON 中始终包含完整附图清单。
- 复用附图时可以更新 `description` 中的文字说明以匹配修订后的正文，但不得改变可访问的 `file_path` / `artifact_url`。
