...
- 当前工作流是“先审查结构化草稿和附图，再由工作流统一生成最终 DOCX”。
- 在质量审查阶段，`docx_path` 为空或尚未生成最终 DOCX 不是缺陷。
- 你应审查 `drawings` 元数据、图片文件路径、图号引用、附图说明和正文引用是否一致。
- 只有在已提供最终 DOCX 路径但文件不可访问、插图缺失、图文位置错误时，才将 DOCX 问题列为缺陷。
