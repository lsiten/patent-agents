        ...
        try:
            ...
            for round_num in range(max_rounds):
                ...
                for agent_id, task_desc in dispatched:
                    ...
                    # 直接调用对应的专业 Agent（支持分段续写）
                    self._logger.info(f"CEO dispatched: {agent_id} → {profile_id}")
                    agent_text = await self._call_agent_with_continuation(service, profile_id, task_desc)
...
请评估已有成果，根据修正建议使用 dispatch_specialist 继续推进流程直到完成。"""
    ...
    async def _call_agent_with_continuation(self, service, profile_id: str, task: str, max_continuations: int = 2) -> str:
        """调用 Agent 并在输出被截断时自动续写

        检测输出是否完整（JSON 闭合），如果截断则追加"继续"调用拼接结果。
        """
        # 第一次调用
        ...
        # 检测是否被截断（有 ```json 开头但没有闭合 ```，或 { 多于 }）
        triple = chr(96) * 3
        has_json_start = (triple + "json") in text or text.strip().startswith("{")
        open_braces = text.count("{") - text.count("}")
        open_brackets = text.count("[") - text.count("]")
        is_truncated = has_json_start and (open_braces > 0 or open_brackets > 0)
        ...
        for i in range(max_continuations):
            self._logger.info(f"Agent output truncated, continuation {i+1}")
            continuation_prompt = "你的上一次输出被截断了，请从截断处继续输出剩余内容（不要重复已输出的部分，直接继续）。"
