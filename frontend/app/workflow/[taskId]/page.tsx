...
import {
  ...
  workflowEventShouldReload,
  workflowProtocolEventToLogEntry,
} from '@/lib/workflowProtocolStore';
...
import { AgentLoopPanel } from '@/components/workflow/AgentLoopPanel';
...
const showDevAgentLoop = process.env.NODE_ENV === 'development';
...
export default function WorkflowPage() {
  ...
  useEffect(() => {
    ...
    let closed = false;
    let logIdCounter = 0;
    const protocolStore = createWorkflowProtocolStore();
    ...
    function connect() {
      ...
      const onSkillSedimented = (e: MessageEvent) => {
        try {
          const parsed = JSON.parse(e.data);
          const data = parsed.data || parsed;
          addLog({
      ...
      const onWorkflowEvent = (e: MessageEvent) => {
        try {
          const parsed = JSON.parse(e.data);
          const snapshot = protocolStore.apply(parsed);
          const event = normalizeWorkflowProtocolEvent(parsed);
          const data = parsed.data || parsed;
          addLog(
            workflowProtocolEventToLogEntry(event, new Date().toISOString()),
            createStableLogId(
              event.agui_type,
              parsed,
              data,
              String(event.display_message || event.message || event.content || snapshot.currentNode || '')
            )
          );
          if (workflowEventShouldReload(event.agui_type) || workflowEventShouldReload(event.event_type || '')) {
            void loadWorkflow();
          }
        } catch {}
      };
      ...
      es.addEventListener('agent.message.end', onWorkflowEvent);
  ...
  const renderAgentOutput = (
    ...
    const getPhaseOutputs = (phaseNames: string[]): Record<string, unknown>[] => {
      ...
      return workflow.phase_history
        .filter((p) => phaseNames.includes(p.phase))
        .map((p) => {
          const outputData = p.output && Object.keys(p.output).length > 0 ? p.output : {};
          return {
            ...outputData,
            __round_meta: {
              phase: p.phase,
        ...
        .filter((o) => {
          const meta = o.__round_meta;
          const hasMetaIssues = Boolean(
            meta &&
            typeof meta === 'object' &&
            !Array.isArray(meta) &&
            (
              (Array.isArray((meta as Record<string, unknown>).issues) && ((meta as Record<string, unknown>).issues as unknown[]).length > 0) ||
              (Array.isArray((meta as Record<string, unknown>).warnings) && ((meta as Record<string, unknown>).warnings as unknown[]).length > 0)
            )
          );
          return Object.keys(o).some((key) => key !== '__round_meta') || hasMetaIssues;
        });
    };
    ...
    switch (type) {
      case 'requirement': {
        const requirementRounds = phaseOutputs.length > 0 ? phaseOutputs : [effectiveOutput];
        return (
          <MultiRoundView
            rounds={requirementRounds}
            label="需求分析"
            renderRound={(data) => <RequirementAnalysisView data={data} />}
          />
        );
      }
      case 'retrieval': {
        const retrievalRounds = phaseOutputs.length > 0 ? phaseOutputs : [effectiveOutput];
        return (
          <MultiRoundView
            rounds={retrievalRounds}
            label="检索"
            renderRound={(data) => <RetrievalReportView data={data} />}
          />
        );
      }
      case 'draft': {
  ...
  return (
    <div className="h-full min-h-0 w-full min-w-0 max-w-full overflow-y-auto overflow-x-hidden bg-surface py-section-lg">
      <div className="mx-auto w-full min-w-0 max-w-screen-2xl px-4 sm:px-6 lg:px-8">
        ...
        {showDevAgentLoop && (
          <div className="mt-xl">
            <AgentLoopPanel workflow={workflow} />
          </div>
        )}
      </div>
    </div>
