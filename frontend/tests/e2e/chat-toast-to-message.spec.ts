...
test('recommendation without structured preflight does not start workflow', async ({ page }) => {
  await page.route(`${API_BASE_PATTERN}/conversations?user_id=default_user`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await mockCreateConversation(page, 'conv-structured-gate', '智能设备显示控制');

  await page.route(`${API_BASE_PATTERN}/conversations/conv-structured-gate/chat/stream`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'text/event-stream',
      body: [
        ...
        '',
        'event: done',
        'data: {"message":{"id":"assistant-start","role":"assistant","content":"技术方案已整理完毕","timestamp":"2026-06-04T00:00:01.000Z"},"has_recommendation":true,"conversation_id":"conv-structured-gate"}',
        '',
      ].join('\n'),
    });
  });
  ...
  await page.route(`${API_BASE_PATTERN}/conversations/conv-structured-gate/create-workflow`, async (route) => {
    createWorkflowCalled = true;
    await route.abort();
  });

  await page.goto('/chat');
  await page.getByTestId('chat-input').fill('智能设备显示控制方案');
  await page.getByTestId('chat-send-button').click();

  await expect(page.getByText('技术方案已基本清晰，可以启动正式专利申请流程。')).toHaveCount(0);
  await expect(page.getByText('请选择申请类型')).toHaveCount(0);
  await expect(page.getByRole('button', { name: '启动专利申请' })).toHaveCount(0);
  expect(createWorkflowCalled).toBe(false);
});
